...
function validaPaquetes(){
	...
	var spanCampo = document.getElementById("spanPrecio");
...
function numerosNegativos(id,idSpan){
	...
	if(campo<0){
		idSpan.className="spanError";
		idSpan.innerHTML = "El " + id.name + " no puede ser negativo";
		id.style.borderColor="#FD8E8E";
		id.style.boxShadow="inset 0 4px 6px rgba(255,0,0,0.4)";
		return false;
	}else{
...
function numerosPermitidos(id, idSpan){
	...
	if(campo != "^(\d|-)?(\d|,)*\.?\d*$"){
		...
		idSpan.className="spanError";
		idSpan.innerHTML = "El " + id.name + " no es válido";
		id.style.borderColor="#FD8E8E";
		id.style.boxShadow="inset 0 4px 6px rgba(255,0,0,0.4)";
		return true;
	}
