...
<html>
	...
	<body class="bodyTabla">
		...
		?>
		<?php
		$query = "SELECT count(*)AS TOTAL FROM Personas WHERE TIPOPERSONA='Buffalo'";
		...
		for($page=1; $page <= $total_pages; $page++){
			...
			<?php }else{ ?>
				<li><a href="paginaBuffalos.php?page_num=<?= $page ?>&page_size=<?=$page_size?>"><?=$page?></a></li>
			<?php 
		}
		}
		 ?>
		</div>
		<form method="get" action"paginaBuffalos.php">
		...
		<?php
		$query2= "SELECT * FROM(
								SELECT ROWNUM RNUM,AUX.* FROM(SELECT PERSONAS.ID_PERSONA,PERSONAS.NOMBRE,PERSONAS.APELLIDOS,PERSONAS.DNI,PERSONAS.FECHANACIMIENTO,PERSONAS.TELEFONO,PERSONAS.EMAIL,PERSONAS.DIRECCION,BUFFALOS.TIPOBUFFALO FROM PERSONAS,BUFFALOS WHERE PERSONAS.ID_PERSONA=BUFFALOS.ID_PERSONA
		...
		?>
		<form method="post" action="procesamientos/procesar_buffalos.php">
			<?php
				if(isset($buffalo) and ($buffalo['id_persona'] == $fila['ID_PERSONA'])){
					...
					<td>
						<input id='cargo' name='cargo' type='text' value='<?=$fila['TIPOBUFFALO'];?>'/>
					</td>
			<?php } else{ ?>
		...
		<td><?=$fila['TIPOBUFFALO']?></td>
		<?php } ?>
		
		<td>
			<?php if(isset($buffalo) and ($buffalo['id_persona'] == $fila['ID_PERSONA'])){?>
		...
		<center><a id="botonAnadir" name="botonAnadir" class="botonAnadir" href="anadirBuffalo.php">Añadir</a></center>
		</div>
	</body>
