...
FECHANACIMIENTO DATE NOT NULL CHECK(FECHANACIMIENTO<to_date('09/01/2016','DD/MM/RR')),
TELEFONO VARCHAR2(9),
EMAIL VARCHAR2(100),
...
ID_PERSONA NUMBER NOT NULL,
TIPOMONITOR VARCHAR2(100) NOT NULL CHECK (TIPOMONITOR IN('Jefe','Normal','Becario')),
PRIMARY KEY(ID_PERSONA),
...
ID_PERSONA NUMBER NOT NULL,
ID_INSTITUTO NUMBER NOT NULL,
TIPOBUFFALO VARCHAR2(100) NOT NULL CHECK(TIPOBUFFALO IN('Alumno','Profesor')),
PRIMARY KEY(ID_PERSONA),
FOREIGN KEY(ID_PERSONA) REFERENCES PERSONAS ON DELETE CASCADE,
...
CREATE SEQUENCE sec_paquete;
CREATE SEQUENCE sec_actividad;
CREATE SEQUENCE sec_instituto;
CREATE SEQUENCE sec_producto;
CREATE SEQUENCE sec_pedido;

/****CREAMOS LOS TRIGGERS***/

/*******IMPORTANTE: EJECUTARLOS UNO A UNO*******/

CREATE OR REPLACE TRIGGER crea_idpersona
...
END;
/
CREATE OR REPLACE TRIGGER crea_idpaquete
BEFORE INSERT ON PAQUETES
FOR EACH ROW
BEGIN
  SELECT sec_paquete.NEXTVAL INTO :NEW.ID_PAQUETE FROM DUAL;
END;
/
CREATE OR REPLACE TRIGGER crea_idactividad
BEFORE INSERT ON ACTIVIDADES
FOR EACH ROW
BEGIN
  SELECT sec_actividad.NEXTVAL INTO :NEW.ID_ACTIVIDAD FROM DUAL;
END;
/
CREATE OR REPLACE TRIGGER crea_idinstituto
BEFORE INSERT ON INSTITUTOS
FOR EACH ROW
BEGIN
  SELECT sec_instituto.NEXTVAL INTO :NEW.ID_INSTITUTO FROM DUAL;
END;
/
CREATE OR REPLACE TRIGGER crea_idproducto
...
END;
/
CREATE OR REPLACE TRIGGER crea_idpedido
