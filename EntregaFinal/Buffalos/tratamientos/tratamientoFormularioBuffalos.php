<?php
	...
	if(isset($_SESSION['formulario'])){
		...
		$formulario['tipoBuffalo'] = $_REQUEST['tipoBuffalo'];
		$_SESSION['formulario']=$formulario;
		$errores = validar($formulario);
	
	if(count($errores) > 0){
		$_SESSION['errores']=$errores;
		Header("Location: ../anadirBuffalo.php");
	}else{
		
		Header("Location: ../exitos/exitoBuffalos.php");
	}
}else{
	Header("Location: ../anadirBuffalo.php");
}
...
function validar($formulario){
	if(empty($formulario['nombre'])){
		...
		if(empty($formulario['tipoBuffalo'])){
			$errores[] = "El tipo de Buffalo no puede ser vacío";
		}
